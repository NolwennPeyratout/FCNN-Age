...
def figPerAge(true, pred, seed, fold_idx):
    true = np.round(true,2)*100
    pred = np.round(pred,2)*100


    age_dict = {}
    for true_age, pred_age in zip(true, pred):
        if true_age not in age_dict:
            age_dict[true_age] = []
        age_dict[true_age].append(pred_age)

    true_ages = sorted(age_dict.keys())
    mean_preds = [np.mean(age_dict[age]) for age in true_ages]
    std_preds = [np.std(age_dict[age]) for age in true_ages]

    counts = [len(age_dict[age]) for age in true_ages]
    ...
    for age, mean, std, count in zip(true_ages, mean_preds, std_preds, counts):
        plt.errorbar(age, mean, yerr=std, fmt='o', color=cmap(norm(count)), capsize=5)
        
    #y=x
    plt.plot([min(true), max(true)], [min(true), max(true)], color="black", linestyle="-")

    #set legend
    sm = cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = plt.colorbar(sm,ax=plt.gca(), label="Number of sample per age")
    plt.xlabel("Real age")
    plt.ylabel("Predicted age")
    plt.title("Predicted age as a funcion of real age")
    plt.grid(True)
    plt.savefig('plots/figPerAge-'+str(seed)+'-'+str(fold_idx)+'.png')
    plt.show()
...
def buildNet(feature_len, TYPE_ACTIVATION, net_name, DEVICE, n_hidden, norm ):
    ...
    if net_name == "dnn":
        net = DNN(feature_len, n_hidden, 0.1, ratio = False).to(DEVICE)  # netBio
    return net
...
def runBestNet(
    ...
    try:
        if nfold != 0:
            ...
            soft.to_csv("{}Labelspred_value{}.csv".format(outputPath, ETA), sep=";", index=0, float_format='%g')
        else:
            soft = pd.DataFrame(
                index_pred_probs,
                columns=["Name", "Initial Value"]
...
def ReadData(
    file_name, doScale=True, doLog=True, doRowNorm = False, doMeanto1 = False
):
    try:
        data_pd = pd.read_csv(
            "../../data/" + str(file_name),
...
def ReadDataCV(
    ...
    if doMeanto1:
        X_train = X_train - np.mean(X_train, axis = 0)
        X_test = X_test - np.mean(X_test, acis = 0)
    if doRowNorm: 
